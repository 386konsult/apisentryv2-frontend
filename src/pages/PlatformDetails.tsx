                ...
                {topThreats.length > 0 ? (
                  <div className="space-y-2">
                    {topThreats.slice(0, 3).map((threat, idx) => {
                      ...
                      return (
                        <div key={idx} onClick={() => navigateTo(`/threat-logs?search=${encodeURIComponent(threat.name.toLowerCase())}&time=all`)} className={`flex items-center gap-3 border border-slate-100 dark:border-blue-900/20 bg-slate-50/50 dark:bg-[#0F1724]/50 p-3 ${Rsub} cursor-pointer hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-colors`}>
